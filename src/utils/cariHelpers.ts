...
export const calculateCariBalance = (
    ...
    // Toplam ödeme tutarı (sadece tahsil edilmiş)
    const totalPayments = customerPayments.reduce((sum, payment) => {
        // Sadece tahsil edilmiş ödemeleri say
        if (payment.status !== 'Tahsil Edildi') return sum;
    ...
    // Bekleyen avanslar (Avans/Önödeme tipindeki tahsil edilmiş ödemeler)
 ...
 */
export const getAvailableAdvance = (
    customerId: string,
    payments: Payment[]
): number => {
    const customerPayments = payments.filter(
    ...
    return customerPayments.reduce((sum, payment) => {
        // Sadece avansları ve tahsil edilmişleri say
        if ((payment as any).paymentType !== 'Avans/Önödeme') return sum;
        if (payment.status !== 'Tahsil Edildi') return sum;
