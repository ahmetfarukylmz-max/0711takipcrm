...
const OrderForm: React.FC<OrderFormProps> = ({
  ...
  const handleFormSubmit = (e: React.MouseEvent<HTMLButtonElement>, status: string) => {
    ...
    // Mevcut sipariş düzenleme ise id'yi ekle
    if (order?.id) {
  ...
  return (
    <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-amber-700 dark:text-amber-200">
                <strong>Sevk Edilmiş Sipariş:</strong> Bu sipariş sevk edildiği için müşteri,
                ürünler ve miktarlar değiştirilemez. Fiyatlar, ödeme şekilleri ve teslim tarihi
                düzenlenebilir.
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t">
        ...
        {/* Taslak Butonu - Sadece yeni siparişte veya zaten taslak ise göster */}
        {(!order?.status || order.status === 'Taslak') && (
          <button
            type="button"
        ...
        <button
          ...
          onClick={(e) =>
            handleFormSubmit(
              e,
              order?.status && order.status !== 'Taslak' ? order.status : 'Bekliyor'
            )
          }
