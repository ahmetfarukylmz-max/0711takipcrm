...
const InquiredProductModal: React.FC<InquiredProductModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    createPurchaseRequest: existingProduct?.createPurchaseRequest || false,
  });

  // Auto-check purchase request if stock is low when product is selected
  React.useEffect(() => {
    if (formData.productId && !existingProduct) {
      const product = products.find((p) => p.id === formData.productId);
      // If product has no stock quantity defined or it is <= 0, suggest purchase request
      if (product && (product.stock_quantity === undefined || product.stock_quantity <= 0)) {
        setFormData((prev) => ({ ...prev, createPurchaseRequest: true }));
      } else {
        setFormData((prev) => ({ ...prev, createPurchaseRequest: false }));
      }
    }
  }, [formData.productId, products, existingProduct]);
  ...
  const handleNewProductSave = async (productData: Partial<Product>) => {
    ...
    if (newProductId) {
      // New products definitely need purchasing
      setFormData((prev) => ({ ...prev, productId: newProductId, createPurchaseRequest: true }));
    }
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const inquiredProduct: any = {
      ...
      createPurchaseRequest: formData.createPurchaseRequest,
    };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <FormInput
            ...
            step="0.01"
          />
          <FormSelect label="Birim" name="unit" value={formData.unit} onChange={handleChange}>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-md border border-blue-100 dark:border-blue-800">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              checked={formData.createPurchaseRequest}
              onChange={handleChange}
              className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500 border-gray-300 dark:border-gray-500 dark:bg-gray-700"
            />
            <div>
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400 block">
                Stokta yoksa veya fiyat araştırması gerekiyorsa işaretleyin.
              </span>
            </div>
          </label>
        </div>
