...
const BottomNav = ({ activePage, setActivePage, onToggleGuide }) => {
  ...
  const moreItems = [
    ...
    {
      label: 'Satınalma',
      onPress: () => setActivePage('Satınalma'),
      ...
      >
        <div className="flex justify-around items-center h-16 px-2">
          {navItems.map(({ page, label, Icon }) => (
            ...
            >
              <Icon
                ...
                aria-hidden="true"
              />
              <span className={`text-xs font-medium ${activePage === page ? 'font-semibold' : ''}`}>
                {label}
              </span>
              {activePage === page && (
                <div
                  className="absolute bottom-0 h-0.5 w-12 bg-blue-600 dark:bg-blue-400 rounded-t-full"
                  aria-hidden="true"
                />
              )}
            </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center flex-1 h-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-inset rounded ${
              isMoreActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'
            }`}
            title={`Daha fazla seçenekler menüsü${isMoreActive ? ' (Şu anda aktif)' : ''}`}
          >
            <MenuIcon
              className={`w-6 h-6 mb-1 transition-transform ${isMoreActive ? 'scale-110' : ''}`}
              aria-hidden="true"
            />
            <span className={`text-xs font-medium ${isMoreActive ? 'font-semibold' : ''}`}>
              Daha Fazla
            </span>
            {isMoreActive && (
              <div
