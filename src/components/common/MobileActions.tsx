...
interface Action {
  ...
  disabled?: boolean;
  tooltip?: string;
  destructive?: boolean; // Alias for variant='danger' compatibility
}
 ...
 */
const MobileActions: React.FC<MobileActionsProps> = ({ actions, direction = 'horizontal' }) => {
  const getButtonClasses = (variant: Action['variant'] = 'secondary') => {
    const baseClasses =
      'flex-1 px-4 py-2.5 rounded-lg font-medium text-sm transition-colors active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2';
