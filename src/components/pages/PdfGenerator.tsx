...
const PdfGenerator = memo<PdfGeneratorProps>(({ doc, customers, products, orders = [], shipments = [] }) => {
    ...
    const relatedOrder = isShipment && orders ? orders.find(o => o.id === (doc as Shipment).orderId) : undefined;
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="flex items-center justify-center w-full">
                                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-gray-400"><span className="font-semibold">Yüklemek için tıklayın</span></p>
                                    </div>
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                />
                                <span className="text-sm text-gray-500 dark:text-gray-400 uppercase">{themeColor}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <label className="flex items-center cursor-pointer">
                                    <input type="checkbox" checked={showProductDescriptions} onChange={() => setShowProductDescriptions(!showProductDescriptions)} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                                    <span className="ml-2 text-sm text-gray-600 dark:text-gray-400">Ürün Açıklamalarını Göster</span>
                                </label>
                                {!isShipment && (
                                    <>
                                        <label className="flex items-center cursor-pointer">
                                            <input type="checkbox" checked={showUnitPrices} onChange={() => setShowUnitPrices(!showUnitPrices)} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                                            <span className="ml-2 text-sm text-gray-600 dark:text-gray-400">Birim Fiyatları Göster</span>
                                        </label>
                                        <label className="flex items-center cursor-pointer">
                                            <input type="checkbox" checked={showVatDetails} onChange={() => setShowVatDetails(!showVatDetails)} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
