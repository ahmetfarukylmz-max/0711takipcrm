...
const DocumentPreview = forwardRef<HTMLDivElement, DocumentPreviewProps>(
    ({ doc, logo, themeColor, showProductDescriptions, showUnitPrices, showVatDetails, customNotes, customers, products, relatedOrder }, ref) => {
        ...
        if (isQuote) {
            ...
            documentId = (doc as Shipment).trackingNumber ? `TRK-${(doc as Shipment).trackingNumber}` : (doc as any).id?.substring(0, 8).toUpperCase(); 
            ...
            <div ref={ref} className="bg-white p-8 max-w-4xl mx-auto text-black h-full flex flex-col">
                ...
                <header className="flex justify-between items-start pb-6 border-b-2 mb-6" style={{ borderColor: themeColor }}>
                    ...
                    <div className="text-right w-1/2">
                        <h2 className="text-4xl font-bold mb-4" style={{ color: themeColor }}>{documentType}</h2>
                ...
                <section className="mb-8">
                    <div className="flex flex-col md:flex-row gap-6">
                        <div className="w-full md:w-1/2">
                            <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Sayın</h3>
                            <div className="p-4 bg-gray-50 rounded-lg border border-gray-100 h-full">
                                <p className="font-bold text-gray-900 text-lg mb-2">{customer?.name || 'Müşteri Bilgisi Bulunamadı'}</p>
                                <p className="text-sm text-gray-600 whitespace-pre-line mb-2">{customer?.address}</p>
                ...
                <div className="mt-auto">
                        ...
                        <section className="flex justify-end mb-8">
                            <div className="w-64 space-y-2">
                                <div className="flex justify-between text-sm py-1 border-b border-gray-100">
                                    ...
                                    <span className="font-medium text-gray-900">{formatCurrency((doc as any).subtotal || 0, (doc as any).currency || 'TRY')}</span>
                                </div>
                                {showVatDetails && (
                                    <div className="flex justify-between text-sm py-1 border-b border-gray-100">
                                        <span className="text-gray-600">KDV (%{(doc as any).vatRate || 20})</span>
                                        <span className="font-medium text-gray-900">{formatCurrency((doc as any).vatAmount || 0, (doc as any).currency || 'TRY')}</span>
                                    </div>
                                )}
                                <div className="flex justify-between py-2 px-3 header-bg header-text rounded-md shadow-sm">
                                    ...
                                    <span className="font-bold">{formatCurrency((doc as any).total_amount || 0, (doc as any).currency || 'TRY')}</span>
                                </div>
                            </div>
                        </section>
                    )}
                    ...
                    {(customNotes || (doc as any).notes) && (
                        <section className="mb-8 border p-4 rounded-lg border-l-4" style={{ borderColor: themeColor, borderLeftColor: themeColor }}>
                            <h3 className="text-xs font-bold mb-2 uppercase tracking-wider section-title">Notlar</h3>
                    ...
                    {isShipment && (
                        <section className="grid grid-cols-2 gap-12 mt-12 border-t pt-8 border-gray-200">
                            ...
                            <div className="text-center">
                                ...
                                <div className="border-t border-gray-300 mx-8 pt-2">
                                    <p className="text-xs text-gray-500">İmza / Kaşe</p>
                                    <p className="text-xs text-gray-400 mt-1">Malzemeleri eksiksiz ve hasarsız teslim aldım.</p>
                                </div>
                            </div>
                        </section>
                    )}
