...
const ShipmentEditForm: React.FC<ShipmentEditFormProps> = ({
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const updatedShipment: Partial<Shipment> = {
      ...
      invoicedAt:
        formData.isInvoiced && !shipment.isInvoiced
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                {shipment.items.map((item, index) => {
                  ...
                  const totalShippedQty = shipments
                    ...
                    .reduce((sum, s) => {
                      const shipmentItem = s.items?.find(
                        (si) =>
                          si.productId === item.productId &&
                          (si.orderItemIndex !== undefined
                            ? si.orderItemIndex === orderItemIndex
                            : true)
                      );
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td className="px-3 py-2 text-sm text-center text-blue-600 dark:text-blue-400 font-semibold">
                        {item.quantity} {unit}
                      </td>
                      <td className="px-3 py-2 text-sm text-center text-gray-700 dark:text-gray-300">
                        {totalShippedQty} {unit}
                      </td>
                      <td
                        className={`px-3 py-2 text-sm text-center font-semibold ${remainingQty > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400'}`}
                      >
                        {remainingQty} {unit}
                      </td>
                    </tr>
 ...
 */
const Shipments = memo<ShipmentsProps>(
  ({
    shipments,
    orders = [],
    products = [],
    ...
    const confirmBatchInvoice = () => {
      ...
      selectedShipments.forEach((shipment) => {
        if (!shipment.isInvoiced) {
          // Only update if not already invoiced (or force update?) - let's just update
          onUpdate({
            ...shipment,
            isInvoiced: true,
            invoicedAt: new Date(bulkInvoiceData.invoiceDate).toISOString(),
            invoiceNotes: bulkInvoiceData.invoiceNo
              ? `${bulkInvoiceData.invoiceNo}`
      ...
      <div>
        ...
        <div className="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
          ...
          <div className="mt-3 text-sm text-gray-600 dark:text-gray-400">
            ...
            {filteredAndSortedShipments.length !== activeShipments.length &&
              ` (${activeShipments.length} toplam)`}
          </div>
        </div>
                ...
                <th className="p-3 text-sm font-semibold tracking-wide text-center text-gray-700 dark:text-gray-300">
                  <input
                    ...
                    checked={
                      ...
                      selectedItems.size === filteredAndSortedShipments.length
                    }
                ...
                filteredAndSortedShipments.map((shipment) => {
                  ...
                  return (
                    <tr key={shipment.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="p-3 text-sm text-center text-gray-900 dark:text-gray-100 font-bold">
                        {customer?.name || 'Bilinmeyen Müşteri'}
                      </td>
                      <td className="p-3 text-sm text-center text-gray-700 dark:text-gray-300">
                        {shipment.carrier || (shipment as any).transporter}
                      </td>
                      <td className="p-3 text-sm text-center text-gray-700 dark:text-gray-300">
                      ...
                      <td className="p-3 text-sm text-center">
                        <span
                          className={`px-3 py-1.5 text-xs font-semibold uppercase tracking-wider rounded-lg whitespace-nowrap ${getStatusClass(shipment.status)}`}
                        >
                          {shipment.status}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center justify-end">
                          <ActionsDropdown
                            actions={[
                              ...
                              shipment.status !== 'İptal Edildi' &&
                              shipment.status !== 'İade Edildi'
                                ? [
                                    {
            ...
            filteredAndSortedShipments.map((shipment) => {
              ...
              return (
                <MobileListItem
                  ...
                  rightContent={
                    <span
                      className={`px-2 py-1 text-xs font-medium uppercase tracking-wider rounded-lg ${getStatusClass(shipment.status)}`}
                    >
                      {shipment.status}
                    </span>
                  }
        ...
        <Modal
          ...
          title={
            currentShipment?.status === 'Teslim Edildi' ? 'Sevkiyat Detayları' : 'Sevkiyat Düzenle'
          }
        >
          {currentShipment && (
            <ShipmentEditForm
        ...
        >
          <div className="space-y-4">
            <p className="text-gray-600 dark:text-gray-300">
              Seçili <strong>{selectedItems.size}</strong> adet sevkiyatı faturalandı olarak
              işaretlemek üzeresiniz.
            </p>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setBulkInvoiceData({ ...bulkInvoiceData, invoiceDate: e.target.value })
                }
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setBulkInvoiceData({ ...bulkInvoiceData, invoiceNo: e.target.value })
                }
