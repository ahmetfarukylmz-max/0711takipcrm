        ...
        {sortedShipments.map((shipment) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    Sipariş: {getShortOrderNumber(order)} • Sevk Tarihi:{' '}
              ...
              {isExpanded && (
                <div className="mt-3 pt-3 border-t border-gray-300 dark:border-gray-600 space-y-3">
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {shipment.items.map((item, idx) => {
                          const originalItem = order?.items?.[item.orderItemIndex];
                          return (
                            <div
                              key={idx}
                              className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-sm rounded p-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700"
                            >
                              <span className="text-gray-900 dark:text-gray-100 font-medium mb-1 sm:mb-0">
                                {item.productName || 'Ürün'}
                              </span>
                              <div className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-400">
                                <span title="Bu sevkiyattaki miktar">
                                  Sevk: <strong>{item.quantity}</strong> {item.unit || 'Adet'}
                                </span>
                                {originalItem && (
                                  <span title="Siparişteki toplam miktar" className="text-gray-400">
                                    (Sipariş: {originalItem.quantity})
                                  </span>
                                )}
                              </div>
                            </div>
                  ...
                  <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1 pt-2 border-t border-gray-200 dark:border-gray-700">
                    <p>
                      <span className="font-medium">Nakliye:</span>{' '}
                      {shipment.carrier || (shipment as any).transporter || 'Belirtilmemiş'}
                    </p>
                    {shipment.tracking_number && (
                      <p>
                        <span className="font-medium">Takip No:</span> {shipment.tracking_number}
                      </p>
                    )}
                    {shipment.notes && (
                      <p>
                        <span className="font-medium">Not:</span> {shipment.notes}
                      </p>
                    )}
                  </div>
              ...
              {/* Collapsed View: Info & Action only */}
              {!isExpanded && (
                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                  <div className="text-xs text-gray-600 dark:text-gray-400 mb-2">
                    {shipment.items?.length || 0} kalem ürün
                  </div>
                  {onMarkAsInvoiced && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onMarkAsInvoiced(shipment.id);
                      }}
                      className="w-full px-3 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-medium rounded-lg transition-colors"
                    >
                      ✅ Fatura Kesildi İşaretle
